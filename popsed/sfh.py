#!/usr/bin/env python
# -*- coding: utf-8 -*-
# From https://github.com/bd-j/prospector/blob/main/prospect/plotting/sfh.py, with minor bug fixed.
...
__all__ = ["params_to_sfh", "parametric_pset",
           ...
           "compute_mass_formed", "sfh_quantiles",
           "sfh_to_cmf"]
...
def parametric_pset(logmass=None, **sfh):
    ...
    vectors = ["mass", "sf_start", "tage", "tau", "const",
               "fburst", "tburst", "sf_trunc", "sf_slope"]
...
def sfh_quantiles(tvec, bins, sfrs, weights=None, q=[16, 50, 84]):
    ...
    sf = np.array([np.interp(tvec, t, s, left=0, right=0)
...
def parametric_mwa_numerical(tau=4, tage=13.7, power=1, n=1000):
    ...
    tavg = np.trapz((t**p)*np.exp(-t/tau), t) / \
        np.trapz(t**(power) * np.exp(-t/tau), t)
    return tage - tavg
...
def compute_mass_formed(times, pset):
    ...
    # the ages at which sfr and formed mass are requested
    tprime = times - pset.sf_start
    ...
    if (pset.sfh == 0):
        ...
        sfr_tau = (tprime > 0.) * (tprime <= ttrunc) * (tprime /
                                                        pset.tau)**(power-1.) * np.exp(-tprime / pset.tau)
    ...
    if ((pset.sfh == 1) or (pset.sfh == 4)):
        ...
        sfr = ((1. - pset.const - pset.fburst) * sfr_tau /
               total_mass_tau + pset.const * sfr_const)
        # We've truncated
        sfr *= (tprime <= ttrunc)
    ...
    # This is the integral of sfr_trunc*(1 - m * (T - Ttrunc)) from Ttrunc to Tz
        ...
        if (ttrunc < 0):
            ...
            total_mass_linear = (Thi > ttrunc) * sfr_trunc * \
                linear_mass(Thi, ttrunc, m)
            mass_linear = (tprime > ttrunc) * sfr_trunc * \
                linear_mass(tprime, ttrunc, m)
            mass_linear[tprime > Tz] = sfr_trunc * linear_mass(Tz, ttrunc, m)
            # SFR in linear portion
            sfr = sfr_trunc * (1 - m * (tprime - ttrunc)) / \
                (total_mass_tau + total_mass_linear)
            sfr *= ((tprime > ttrunc) & (tprime <= Tz))
            # add portion for tau
            sfr[tprime <= ttrunc] = sfr_tau[tprime <= ttrunc] / \
                (total_mass_tau + total_mass_linear)
