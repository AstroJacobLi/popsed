...
for i_bin in range(0, 5):
    ...
    pca = SpectrumPCA(n_comp,
                      [f'./train_sed_NMF_ZH/nmf_zh_seds/fsps.NMF_ZH.v0.2.log10spectrum.seed{k+1}{str_wbin}.npy' for k in range(
                          0, 4)]
                      )
    pca.scale_spectra()
    pca.train_pca(chunk_size=1000)
    del pca.normalized_logspec
    pca.save(f'./train_sed_NMF_ZH/nmf_zh_seds/fsps.NMF_ZH.pca_trained{str_wbin}.pkl')
