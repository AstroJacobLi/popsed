...
def train_NDEs(seed_low, seed_high, num_transforms=5, num_bins=40, hidden_features=100,
    ...
    for seed in range(seed_low, seed_high):
        with open('./pretrained_nde.pkl', 'rb') as f:
            NDE_theta = pickle.load(f)

        NDE_theta.output_dir = output_dir

        if (NDE_theta.output_dir is not None):
            try:
                if not os.path.exists(NDE_theta.output_dir):
                    os.makedirs(NDE_theta.output_dir)
            except:
                pass
...
# python train_nde_cdf_nsa_pretrained.py --seed_low=0 --seed_high=5 --num_transforms=10 --num_bins=50 --hidden_features=50 \
