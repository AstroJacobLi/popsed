...
def _train_emu(i_bin, name='NMF_ZH', file_low=0, file_high=15, batch_size=256, rounds=6):
    ...
    params = np.concatenate([np.load(f'./train_sed_{name}/{name.lower()}_seds/fsps.{name}.v{training_sed_version}.theta_unt.seed{i+1}.npy')
    ...
    wave = np.load(
        f'./train_sed_{name}/{name.lower()}_seds/fsps.wavelength.npy')
    wave_bin = [
        (wave >= 1000) & (wave < 2000),
        (wave >= 2000) & (wave < 3600),
        (wave >= 3600) & (wave < 5500),
    ...
    fspecs = [f'./train_sed_{name}/{name.lower()}_seds/fsps.{name}.v{training_sed_version}.log10spectrum.seed{i+1}{str_wbin}.npy'
              for i in range(int(file_low), int(file_high))]
    pca_coeff = []
    for file in fspecs:
